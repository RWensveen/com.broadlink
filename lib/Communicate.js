...
class Communicate  {
	 ...
	 * options = { count: this.count,
	    		   ...
	    		   key: this.key   // might be null
	    		   id:  this.id    // might be null
	             }
	...
	configure( options ) {
		...
		this.mac       = Util.hexToArr(options.mac);
		this.ipAddress = options.ipAddress;
		
		if( options.id) {
			this.id    = Util.hexToArr(options.id);	
		}
		else {
		...
		if(( options.key ) && ( options.key.length == 32)) {
			this.key = Util.hexToArr(options.key);
		}
		else {
	...
	auth() {
	    ...
	    return this.send_packet(0x65, payload)
	    	.then( function( response ) {
	    				...
	    				that.id = id;
	    				that.key = key;
	    				let authData = { id:  Util.arrToHex(id),
	    								 key: Util.arrToHex(key) 
	    				}
