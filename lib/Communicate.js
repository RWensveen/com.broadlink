...
class Communicate  {
	...
	*/
	...
	emitResult( success, msg ) {
		...
		let q = this.sendQueue.shift() // get & remove first elem in queue
		this.sendNext( true );
		if(success) {
			q.resolve( msg );
		}
...
// startNow = true : always start if any elem in queue
//  false: only start if 1 elem in queue (which was just added)
	sendNext( startNow ) {
		//Util.debugLog("communicate.sendNext - " +startNow)
		
		if( ( ( startNow ) && ( this.sendQueue.length > 0 ) ) ||
			( ( ! startNow ) && ( this.sendQueue.length == 1 ) ) )
		{
			if( this.dgramSocket === undefined ) {
				...
				this.dgramSocket.on('error', (err) => {
					...
					this.emitResult( false, Error(err.stack));
				});

				this.dgramSocket.on('message', (msg, rinfo) => {
					//Util.debugLog('<== dgramSocket - got message with size='+rinfo.size);
					let rsp = { data:    msg,   // msg is type Buffer
					            size:    rinfo.size,
					            address: rinfo.address,
					            port:    rinfo.port
					};
					this.emitResult( true, rsp );
				});
			}

			this.dgramSocket.send( this.sendQueue[0].packet, this.sendQueue[0].port, this.sendQueue[0].ipaddress );
			this.tm = setTimeout( function() {
				Util.debugLog('**> dgramSocket timeout');
				this.terminateSocket()
				this.emitResult( false, Error(Homey.__('errors.sending timeout')));
			}.bind(this), this.sendQueue[0].timeout * 1000 );  // timeout in [msec]
		}
	}
	...
	*/
	...
	async checkRFData() {
		...
		if ( resp[0] != 1 ) {
			throw new Error(Homey.__('errors.no_key_detected'))
		}
	}
	...
	*/
	async read_status() {
		...
		if( response.error == 0 ) {
			...
			throw new Error(Homey.__('errors.invalid_response'))
		}
	}
