...
class Communicate  {
    ...
    */
	 ...
	 * return: [Promise]
	 */
	 sendto__(packet, ipaddress, port, timeout ) {
     ...
     */
     async send_packet(command, version, payload) {
        ...
        Util.debugLog('->send_packet: pact='+Util.asHex(packet));
        // send packet
        let response = await this.sendto( packet, this.ipAddress, 80, 5 )
        Util.debugLog('->send_packet: resp='+Util.asHex(response.data))

        // call the garbage collector. 
        // Do this here, as all device.js will use this function to communicate with their paired devices.
        try {
			global.gc();
		} catch (e) { }
    ...
    auth() {
        ...
        var that = this;
        return this.send_packet(0x65, false, payload)
    ...
    discover( timeout, local_ip_address, device_ip_address ) {
       ...
       return this.sendto( packet, device_ip_address, 80, timeout )
            ...
            .catch( function( err ) {
                	Util.debugLog('**> discover: catch = '+err);
                    throw err;
                });
    }
    ...
    enter_learning() {
        ...
        return this.send_packet(0x6a, false, payload)
            .then ( response => {
    ...
    _check_data( payload ) {
        ...
        return new Promise( function(resolve, reject ) {
            ...
            let tm = setInterval( function() {
                Util.debugLog('_check_data - repeat='+repeatCount)
                that.send_packet(0x6a, false, payload)
                    .then ( response => {
                    	let err = -1
                        if( response ) {
    ...
    enterRFSweep() {
        var payload = new Uint8Array(16);
        payload[0] = 0x19;
        Util.debugLog('->enterRFSweep');
        return this.send_packet(0x6a, true, payload)
            .then ( response => {
                return response;
            })
            .catch( err => {
            ...
            that.send_packet(0x6a, false, payload)
                .then ( response => {
                    ...
                    if( err == 0 ) {
                    	...
                    	Util.debugLog('**> read_status: ongeldig antwoord van apparaat');
                        reject( Homey.__('errors.invalid_response'))
                    }
                }, rejection => {
                	Util.debugLog('**> read_status: reject - ' + rejection);
                    reject( rejection );
                })
                .catch( err => {
                    throw err;
            ...
            that.send_packet(0x6a, false, payload)
                .then ( response => {
                    ...
                    if( err == 0 ) {
                    	...
                    	Util.debugLog('**> sp2_get_energy: ongeldig antwoord van apparaat');
                        reject( Homey.__('errors.invalid_response'))
                    }
                }, rejection => {
                	Util.debugLog('**> read_status: reject - '+rejection);
    ...
    */
        ...
        return this.send_packet(0x6a, false, payload)
            .then ( response => {
                ...
                if( err == 0 ) {
                	...
                	Util.debugLog('**> mp1_check_power: ongeldig antwoord van apparaat');
                    Promise.reject( Homey.__('errors.invalid_response'))
                }
            }, rejection => {
            	Util.debugLog('**> mp1_check_power: reject' + rejection);
    ...
    sp1_set_power_state(mode) {
        ...
        return this.send_packet(0x66, false, payload)
            .then ( response => {
                return response;
            }, rejection => {
                reject( rejection );
