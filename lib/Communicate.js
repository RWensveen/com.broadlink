...
class Communicate  {
	...
	read_status() {
		...
		return new Promise( function(resolve, reject ) {
			...
			that.send_packet(0x6a, payload)
				.then ( response => {
					//Util.debugLog("==> read_status.response: " + JSON.stringify(response));
					let err = response[0x22] | (response[0x23] << 8)
					if( err == 0 ) {
						let payload = that.decrypt(response.slice(0x38))  // remove 0x38 bytes from start of array
						let p = payload.slice(0x04)					   // remove first 4 bytes of array
				...
				.catch( err => {
					//Util.debugLog("**> read_status.catch" + err);
					throw err;
				})
		})
	}
	 ...
	 */
	setPowerState(state) {
		//Util.debugLog('==>setPowerState - ' + state );
		...
		return this.send_packet(0x6a, payload)
			...
			.catch( err => {
				//Util.debugLog('<==setPowerState - catch ' + err );
				throw err;
			})
	}
	...
	sp2_get_energy() {
		...
		return new Promise( function(resolve, reject ) {
			...
			that.send_packet(0x6a, payload)
				.then ( response => {
					//Util.debugLog('sp2_get_energy='+Util.asHex(response));
	...
	*/
	mp1_check_power() {
		...
		return new Promise( function(resolve, reject ) {
			...
			that.send_packet(0x6a, payload)
				.then ( response => {
					//Util.debugLog('mp1_check_power='+Util.asHex(response));
	...
	sp1_set_power_state(mode) {
		...
		return this.send_packet(0x66, payload)
			...
			.catch( err => {
				//Util.debugLog('<==sp1_set_power_state - catch ' + err );
				throw err;
		})
