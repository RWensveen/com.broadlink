...
class Communicate  {
	...
	async sendto(packet, ipaddress, port, timeout ) {
		...
		if( this.dgramSocket === undefined ) {
			...
			this.dgramSocket.on('message', (msg, rinfo) => {
				...
				//Util.debugLog('Communicate.sendto - size='+rsp['size'] +' - address='+rsp['address'] + ' - port='+rsp['port']);
	...
	*/
	async send_packet(command, version, payload) {
		...
		Util.debugLog('->send_packet: sendto = '+this.ipAddress+': pact='+Util.asHex(packet));
		// send packet
		let response = await this.sendto( packet, this.ipAddress, 80, 10 )
		Util.debugLog('->send_packet: resp = '+response['address']+':resp='+Util.asHex(response.data))
	...
	*/
	async auth() {
		...
		var payload = this.decrypt(response.slice(0x38))
		if( !payload ) {
	...
	*/
	async enter_learning() {
		...
		return await this.send_packet(0x6a, false, payload);
	}
					...
					.then ( 
						function( response ) {
							...
							if( err == 0 ) {
								clearInterval(tm)
								tm = null
								let payload = this.decrypt(response.slice(0x38))  // remove 0x38 bytes from start of array
	...
	*/
	async enterRFSweep() {
		...
		return await this.send_packet(0x6a, true, payload)
	}
	...
	*/
	async read_status() {
		...
		if( err == 0 ) {
			let payload = this.decrypt(response.slice(0x38));  // remove 0x38 bytes from start of array
			return payload.slice(0x04);						// remove first 4 bytes of array
		}
		else {
